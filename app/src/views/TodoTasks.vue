        ...
        <template v-else>
            <div class="flex items-center mb-8">
                <RouterLink :to="{ name: 'index' }" class="-ml-2">
                    <svg class="h-5 w-5 text-gray-500 mr-2" viewBox="0 0 20 20" fill="none"
            ...
            <div class="w-100 flex justify-end mb-3">
                <div>

                    <input v-model="conclusionTask" type="date" @change="updateDate"
                        class="bg-gray-100 w-2/4 placeholder-gray-700 text-gray-500 font-light focus:outline-none block w-full appearance-none leading-normal pr-3">
                </div>
            </div>
            <form class="flex items-center px-4 bg-gray-900 h-15 rounded-sm border-l-2 border-green-400 mb-3"
                ...
                <input v-model="newTask" placeholder="Adicione um novo item ..." type="text"
            ...
            <div v-if="todo.tasks.length" class="bg-gray-300 rounded-sm">
                <TodoTaskCard v-for="task in todo.tasks" :key="task.id" :task="task" @afterDeleting="afterDeleting" />
            </div>
